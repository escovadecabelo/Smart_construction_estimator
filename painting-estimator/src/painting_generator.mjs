    ...
    const styles = `
        <style>
            ...
            * { margin: 0; padding: 0; box-sizing: border-box; }
            ...
            .total-section { background: var(--primary); padding: 50px; border-radius: 30px; color: var(--white); display: flex; justify-content: space-between; align-items: center; margin: 40px 0; box-shadow: 0 25px 50px -12px rgba(15, 23, 42, 0.4); }
            .total-section .value { font-size: 3.2rem; font-weight: 900; color: #60a5fa; letter-spacing: -2px; }
            .terms-box { background: #fafafa; border: 1px dashed var(--border); padding: 30px; border-radius: 20px; font-size: 0.8rem; color: var(--text-muted); margin-bottom: 40px; }
...
<head>
    <meta charset="UTF-8">
    <title>Painting Proposal | Mardegan Construction</title>
    ...
    <div class="proposal-container">
        <header>
            <div class="company-meta">
                ...
                <p class="sub">High-End Finishing Specialists</p>
                <p class="address">904 Valleybrook Dr. Lewisville - TX, 75067</p>
            </div>
            <div class="proposal-meta">
                <h1>Service Proposal</h1>
                <p class="ref">REF: PNT-${Math.floor(Math.random() * 8999) + 1000}</p>
            </div>
        </header>
        ...
        <div class="intro-grid">
            ...
            <div class="info-group"><label>Valid Until</label><p>30 Days</p></div>
        </div>

        <h2><span>01</span> Material Specification</h2>
        <div class="color-palette">
            ${data.paintCodes ? data.paintCodes.map(c => `
                <div class="color-item">
                    <div class="swatch" style="background: ${c.name === 'Tricorn Black' ? '#2f2f2f' : (c.name === 'Urbane Bronze' ? '#54504a' : '#f9f9f9')}"></div>
                    <strong>${c.code} ${c.name}</strong>
                    <span>${c.finish} / ${c.area}</span>
                </div>
            `).join('') : ''}
        </div>

        <h2><span>02</span> Investment Ledger</h2>
        <table class="ledger-table">
            <thead><tr><th>Scope Item</th><th style="text-align: right;">Subtotal</th></tr></thead>
            <tbody>
                ${data.items.map(item => `
                    <tr>
                        <td>
        ...
        <div class="total-section">
            <div class="total-label">
                ...
                <p>Premium materials & master labor included.</p>
            </div>
            <div class="value">${formatCurrency(data.totalBaseBid)}</div>
        </div>

        <div class="terms-box">
            <p><strong>Standards:</strong> All execution follows Mardegan's high-fidelity finishing protocol. This bid includes all necessary prep, protection, and premium Sherwin Williams coatings.</p>
        </div>
